/* Run from backend after building: node test/collections-db-smoke.cjs
...
async function main() {
  ...
  try {
    await db.$transaction(async tx => {
      ...
      const provider = { $queryRaw: tx.$queryRaw.bind(tx), payment: tx.payment, loanInstallment: tx.loanInstallment, user: tx.user, client: tx.client, loan: tx.loan, session: tx.session, $transaction: async op => typeof op === 'function' ? op(tx) : Promise.all(op) };
      ...
      try {
        ...
        const collectionName = `Cobranza_${randomUUID()}`;
        await tx.client.update({ where: { id: client.id }, data: { fullName: collectionName } });
        const baseDate = businessDate();
        function dateOffset(days) { const date = new Date(`${baseDate}T00:00:00Z`); date.setUTCDate(date.getUTCDate() + days); return date.toISOString().slice(0, 10); }
        const collectionLoans = [];
        for (const offset of [-14, 1, 7, 8]) {
          const result = await api.post('/loans').set('Cookie', cookie).set('Origin', origin).send({ ...input, requestId: randomUUID(), firstPaymentDate: dateOffset(offset) }).expect(201);
          collectionLoans.push(result.body.id);
        }
        await pay({ ...paymentInput, loanId: collectionLoans[0], requestId: randomUUID(), amountCents: 50000 }).expect(201);
        const collections = (status, page = 1, limit = 20) => api.get(`/collections?search=${encodeURIComponent(collectionName)}&status=${status}&page=${page}&limit=${limit}`).set('Cookie', cookie);
        await api.get('/collections').expect(401); checks++;
        await api.get('/collections?status=UNKNOWN').set('Cookie', cookie).expect(400); checks++;
        await api.get('/collections?page=0').set('Cookie', cookie).expect(400); checks++;
        await api.get('/collections?limit=101').set('Cookie', cookie).expect(400); checks++;
        const overdue = await collections('OVERDUE').expect(200);
        assert.equal(overdue.body.asOf, baseDate);
        assert.equal(overdue.body.items.length, 2);
        assert.equal(overdue.body.items[0].balanceCents, 50000);
        assert.equal(overdue.body.items[0].paidCents, 50000);
        assert.equal(overdue.body.items[0].daysOverdue, 14);
        assert.equal(overdue.body.items[1].daysOverdue, 7);
        assert.equal(overdue.body.summary.overdueCents, 150000);
        assert.equal(overdue.body.summary.todayCents, 100000);
        assert.equal(overdue.body.summary.nextSevenDaysCents, 300000);
        assert.equal(overdue.body.summary.outstandingCents, 5550000);
        assert.equal(overdue.body.summary.loanCount, 4); checks++;
        const todayView = await collections('TODAY').expect(200);
        assert.equal(todayView.body.items.length, 1);
        assert.equal(todayView.body.items[0].daysOverdue, 0);
        assert.equal(todayView.body.items[0].status, 'TODAY'); checks++;
        const upcoming = await collections('NEXT_7').expect(200);
        assert.equal(upcoming.body.items.length, 3);
        assert.ok(upcoming.body.items.every(i => i.dueDate > baseDate && i.dueDate <= dateOffset(7)));
        assert.ok(!upcoming.body.items.some(i => i.loanId === collectionLoans[3])); checks++;
        const page1 = await collections('ALL', 1, 2).expect(200);
        const page2 = await collections('ALL', 2, 2).expect(200);
        assert.equal(page1.body.pagination.total, 56);
        assert.equal(page1.body.pagination.totalPages, 28);
        assert.equal(page1.body.items.length, 2);
        assert.ok(page1.body.items.every(a => !page2.body.items.some(b => a.id === b.id)));
        assert.deepEqual(page1.body.summary, page2.body.summary); checks++;
        const noMatch = await api.get(`/collections?search=${encodeURIComponent(collectionName + '%')}&status=ALL`).set('Cookie', cookie).expect(200);
        assert.equal(noMatch.body.pagination.total, 0);
        assert.equal(noMatch.body.summary.outstandingCents, 0); checks++;
        const apostrophe = await api.get(`/collections?search=${encodeURIComponent("' OR 1=1 --")}&status=ALL`).set('Cookie', cookie).expect(200);
        assert.equal(apostrophe.body.pagination.total, 0); checks++;
        for (const role of ['COLLECTOR', 'VIEWER']) {
          await tx.user.update({ where: { id: user.id }, data: { role } });
          await collections('ALL').expect(200); checks++;
        }
        await tx.user.update({ where: { id: user.id }, data: { role: 'ADMIN' } });
        await pay({ ...paymentInput, loanId: collectionLoans[0], requestId: randomUUID(), amountCents: 150000 }).expect(201);
        const caughtUp = await collections('OVERDUE').expect(200);
        assert.equal(caughtUp.body.items.length, 0);
        assert.equal(caughtUp.body.summary.overdueCents, 0);
        assert.equal(caughtUp.body.summary.todayCents, 100000); checks++;
        await pay({ ...paymentInput, loanId: collectionLoans[0], requestId: randomUUID(), amountCents: 1200000 }).expect(201);
        const paidOff = await collections('ALL').expect(200);
        assert.equal(paidOff.body.pagination.total, 42);
        assert.equal(paidOff.body.summary.loanCount, 3);
        assert.equal(paidOff.body.summary.todayCents, 0);
        assert.equal(paidOff.body.summary.nextSevenDaysCents, 200000);
        assert.ok(paidOff.body.items.every(i => i.loanId !== collectionLoans[0])); checks++;
        ...
        assert.equal(await tx.loan.count({ where: { clientId: client.id } }), 5);
        const inactive = await collections('ALL').expect(200);
        assert.equal(inactive.body.pagination.total, 42);
        assert.ok(inactive.body.items.every(i => i.client.isActive === false)); checks++;
